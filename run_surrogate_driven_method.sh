echo "Running the surrogate-driven method"
...
surrTxtFile=${datadir}/surrogate_rpm_grad.txt
outputdir=output/surr_driven/
mkdir -p ${outputdir}

echo "fitting the motion model and reconstructing the motion-compensated image..."

./runSupremo -dType 1 -mcrType 3 -optimiserType 0 -maxFitIt 5 -maxSwitchIt 6 -nThreads 12 \
...
./animate 12 ${surrTxtFile} ${numberOfSurrogates} ${outputdir}/motionModel_t00.nii.gz  3 ${outputdir}/finalImage.nii.gz ${refStateImg} ${outputdir}/estimated_volumes/ 
...
./animate 12 ${surrTxtFile} ${numberOfSurrogates} ${outputdir}/motionModel_t00.nii.gz  3 ${datadir}/ref_tumormask.nii.gz ${refStateImg} ${outputdir}/estimated_tumormasks/
